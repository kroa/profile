...
**출처 (모두 self-host — 핫링크 안 함)**
- 자격증 배지 6종: Credly 공식 발급기관 페이지의 배지 이미지 (192x192 축소)
- `logo-shinsegae-inc.png`: 신세계아이앤씨 공식 사이트 CI (364x46, 흰 배경)
- `logo-gaeasoft.png`: 지어소프트 공식 사이트 CI 컴러 버전 (128x41)

> 로고는 흰 칩 안에 표시됩니다 — 브랜드 색이 다크 배경에서 죽지 않도록 하기 위함입니다.
> ⚠ 회사 로고·자격증 배지는 각 상표권자 소유입니다. 본인 경력·취득 사실 표기 목적의
> 명목적 사용 범위이며, 변형하지 말고 원형 그대로 쓰세요.
> 자격증 배지는 본인 Credly 계정의 발급본으로 교체하면 검증 링크까지 연결할 수 있습니다.
